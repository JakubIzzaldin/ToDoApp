...
export const TodoCard = ({allTodos, title, cardId}: TodoCardProps) => {
  ...
  const handleAddNewTodo = () => {
    dispatch(addTodo({text: newTodo, priority: 'none', cardId, isDone: false}));
    setNewTodo('');
  };
  const appTabsConfig: AppTabsProps['tabsConfig'] = [
    {
      tabContent: <VStack>{getTodosWidgetElements(allTodos, cardId, 'all')}</VStack>,
      tabText: 'All',
    },
    {
      tabContent: <VStack>{getTodosWidgetElements(allTodos, cardId, 'todo')}</VStack>,
      tabText: 'To do',
    },
    {
      tabContent: <VStack>{getTodosWidgetElements(allTodos, cardId, 'done')}</VStack>,
      tabText: 'Done',
    },
  ];
  return (
    <Card p={'0px'} minWidth={sizes.addCardButtonWidth}>
      <CardHeader p={'8px'} height={'56px'}>
        <TodoCardHeader text={title} />
      </CardHeader>
